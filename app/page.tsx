...
export default function HomePage() {
  return (
    <main className="w-full bg-white font-['Noto_Sans_KR',sans-serif] text-neutral-900">
      ...
      <section
        id="about"
        className="grid grid-cols-1 items-center gap-10 px-4 py-16 tb:px-10 tb:py-20 dt:grid-cols-[1.4fr_1fr] dt:gap-16 dt:px-[max(40px,calc((100vw-1200px)/2))] dt:py-24"
      >
        <div className="flex flex-col gap-5">
          ...
          <div className="flex flex-wrap gap-2.5 pt-1.5">
            {CONTENT.about.tags.map((t) => (
              <Text as="span" key={t} size="sm" weight="medium" className="rounded-full bg-neutral-200 px-4 py-2 text-neutral-700">
                {t}
              </Text>
            ))}
          </div>
          ...
          >
            <Text as="span" size="sm" weight="bold">{CONTENT.about.buttonLabel}</Text>
          </Link>
        </div>
      ...
      <section className="bg-neutral-700 px-4 py-16 text-white tb:px-10 tb:py-20 dt:px-[max(40px,calc((100vw-1200px)/2))] dt:py-[88px]">
        <div className="mb-14 flex flex-col items-center gap-3.5 text-center">
          <Text as="span" size="xs" weight="bold" className="uppercase tracking-[0.22em] text-neutral-300">{CONTENT.results.eyebrow}</Text>
          <Text as="h2" size="3xl" weight="bold" className={`${HEADING} text-white`}>{CONTENT.results.title}</Text>
          <Text size="md" className="max-w-[620px] leading-relaxed text-white/60">{CONTENT.results.description}</Text>
        </div>
          ...
          {CONTENT.results.items.map((s, index) => (
            ...
            <div className="flex h-full flex-col items-center gap-2 rounded-2xl bg-white px-4 py-8 shadow-lg">
              <Text as="span" size="display" className="font-['Gowun_Batang',serif] leading-none text-neutral-700">
                {s.value}
                {s.unit ? <Text as="span" size="2xl" className="ml-1">{s.unit}</Text> : null}
              </Text>
              <Text as="span" size="sm" weight="bold" className="text-neutral-800">{s.label}</Text>
              <Text as="span" size="xs" className="text-neutral-500">{s.note}</Text>
            </div>
            </RevealOnScroll>
          ))}
        </div>
      ...
      <section
        id="reviews"
        className="overflow-hidden bg-neutral-700 px-4 py-16 text-white tb:px-10 tb:py-20 dt:px-[max(40px,calc((100vw-1200px)/2))] dt:pb-24 dt:pt-20"
      >
        <div className="mb-8 flex flex-col items-start justify-between gap-4 tb:mb-10 tb:flex-row tb:items-end">
          ...
          >
            <Text as="span" size="sm" weight="medium">{CONTENT.reviews.buttonLabel}</Text>
          </Link>
        </div>
          ...
          {CONTENT.reviews.items.map((r, index) => (
            ...
            >
              <div className="flex items-start justify-between gap-2.5">
                <Text as="span" size="xs" weight="bold" className="uppercase tracking-widest opacity-60">{r.tag}</Text>
                <Text as="span" size="lg" aria-hidden className="font-['Gowun_Batang',serif] leading-none opacity-35">
                  ❞
                </Text>
              </div>
              <Text as="blockquote" size="sm" weight="medium" className="m-0 leading-relaxed">{r.body}</Text>
              <Text as="figcaption" size="xs" className="opacity-55">{r.meta}</Text>
            </figure>
            </RevealOnScroll>
          ))}
        </div>
        ...
        <div className="rounded-2xl bg-neutral-100 px-6 py-12 tb:px-10 dt:px-14 dt:py-16">
          <div className="mb-10 flex flex-col items-start gap-4">
            <div className="flex flex-col gap-3">
              <Text as="span" size="xs" weight="bold" className="uppercase tracking-[0.2em] text-neutral-500">{CONTENT.cases.eyebrow}</Text>
            ...
            <Text size="md" className="max-w-[560px] leading-relaxed text-neutral-600">{CONTENT.cases.description}</Text>
          </div>
              ...
              >
                ...
                <div className="flex items-center justify-between gap-2 px-1.5 pb-2">
                  <div className="flex flex-col gap-1.5">
                    <Text as="h3" size="lg" weight="bold" className={`${HEADING} leading-snug`}>{c.title}</Text>
                    <Text as="span" size="sm" className="text-neutral-500">{c.meta}</Text>
                  </div>
                  <Text as="span" size="lg" className="text-neutral-400 transition group-hover:translate-x-1 group-hover:text-neutral-700">→</Text>
                </div>
              </Link>
              </RevealOnScroll>
            ))}
      ...
      >
        ...
        <Text size="md" className="max-w-[520px] leading-relaxed text-neutral-700">{CONTENT.cta.description}</Text>
        <Link
          href="/contact"
        ...
        >
          <Text as="span" size="lg" weight="bold">{CONTENT.cta.buttonLabel}</Text>
        </Link>
      </section>
      ...
      <footer className="flex flex-col items-center justify-between gap-4 bg-neutral-800 px-4 py-8 text-sm text-white/55 tb:flex-row tb:px-10 dt:px-[max(40px,calc((100vw-1200px)/2))] dt:py-11">
        <Text as="span" size="md" weight="bold" className="font-['Gowun_Batang',serif] text-white">{CONTENT.footer.brand}</Text>
        <Text as="span" size="sm">{CONTENT.footer.copyright}</Text>
      </footer>
    </main>
  );
}
