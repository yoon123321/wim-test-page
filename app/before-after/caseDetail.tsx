/**
 * 감량 사례 상세의 공통 부품 — wim-homepage-2 의 detail 섹션들을 한 파일로 옮긴 것.
 * 사례별 페이지(case-01, case-02 …)는 자기 DETAIL 데이터와 히어로만 갖고,
 * 나머지 섹션은 여기 CaseDetailPage 가 그린다.
 * 원본과 다른 점: 클릭 추적 없음, 상담 폼 대신 CONSULT_HREF 로 이동, 후기 캐러셀은 목록에만 둔다.
 */
...
export type BnaCaseMetricCard = {
  ...
  /** vfa 그래프에서 점을 찍을 나이 */
  age?: number;
  /** AFTER 막대만 이 비율(%)만큼 더 올린다 — 수치 차이가 작아 눈에 안 띌 때 */
  afterBarBoostPercent?: number;
...
export function CaseHeroSection({
  ...
  /** PC 에서 사진이 서는 높이(px). 전신이냐 상반신이냐에 따라 사례별로 다르다 */
  photoHeight?: number;
  /** 모바일 배치를 직접 그리는 사례(Case02HeroSection)는 false 로 꺼서 마크업이 겹치지 않게 한다 */
  showMobile?: boolean;
}) {
  /** 감량 전·후 두 장을 나란히 세울지 */
    ...
    <section className="relative">
        ...
        <div className="mt-6 flex items-end justify-center">
          {hero.images.map((photo, index) => (
            <img
              ...
              className={
                isPair
                  ? `relative h-[340px] w-auto object-contain object-bottom tb:h-[520px] ${index > 0 ? '-ml-4 tb:-ml-8' : ''}`
                  : 'h-[460px] w-auto max-w-full object-contain object-bottom tb:h-[600px]'
              }
            />
          ))}
        </div>
      ...
      <Container className="hidden pb-14 pt-4 tb:pb-20 tb:pt-8 dt:block">
        <div className="grid grid-cols-1 gap-10 dt:grid-cols-2 dt:grid-rows-[auto_1fr] dt:gap-0">
          ...
          <div
            className="relative z-10 flex items-end justify-center dt:col-start-2 dt:row-span-2 dt:row-start-1"
            style={{ '--hero-photo-h': `${photoHeight}px` } as CSSProperties}
          >
            {hero.images.map((photo, index) => (
              <img
                key={photo.src}
                ...
                className={
                  isPair
                    ? `relative h-[340px] w-auto object-contain object-bottom tb:h-[520px] dt:h-[640px] ${index > 0 ? '-ml-4 tb:-ml-8' : ''}`
                    : 'h-[380px] w-auto max-w-full object-contain object-bottom tb:h-[560px] dt:h-[var(--hero-photo-h)] dt:w-auto dt:max-w-none'
                }
              />
            ))}
          </div>
...
export function CaseCtaBandSection({
  ctaBand,
  emphasized = false
}: {
...
const CHART_ROWS = 6;
/** 막대 끝이 차트 윗단에 붙지 않도록 큰 값도 이 칸까지만 채운다 */
const BAR_MAX_ROWS = 5;
const BAR_MAX_PERCENT = (BAR_MAX_ROWS / CHART_ROWS) * 100;
/** 막대 두 개의 가로 위치(%) */
...
function BeforeAfterChart({
  ...
  const height = (value: number, key?: 'before' | 'after') => {
    const base = max > 0 ? (value / max) * BAR_MAX_PERCENT : 0;
    return key === 'after'
      ? Math.min(base + (card.afterBarBoostPercent ?? 0), 100)
      : base;
  };
  const bars = [
  ...
  return (
    <div className="mt-6 flex min-h-0 flex-1 flex-col tb:mt-auto tb:block tb:flex-none tb:pt-8">
      <div className="relative min-h-0 flex-1 tb:h-[190px] tb:flex-none">
        ...
        >
          <polygon
            points={`${BAR_X.before},${100 - height(card.before, 'before')} ${BAR_X.after},${100 - height(card.after, 'after')} ${BAR_X.after},100 ${BAR_X.before},100`}
            fill="#E4F1DF"
            fillOpacity={0.6}
          />
        </svg>
        ...
        <div aria-hidden="true" className="pointer-events-none absolute inset-0 z-20 flex flex-col justify-between">
          {Array.from({ length: CHART_ROWS + 1 }, (_, index) => (
        ...
        {bars.map((bar) => (
          <div
            ...
            style={{ left: `${bar.x}%`, height: `${height(bar.value, bar.key)}%` }}
          >
            <span
 ...
 */
export function CaseBodyChangeSection({
  change,
  metrics
}: {
...
/**
 * case-02 히어로 — 노트북을 든 상반신 한 장.
 *  모바일: 이름 → 제목 → 수치 → 워터마크 → 사진(화면 폭 꽉 채움)
 *  PC    : case-01 과 같은 좌우 배치. 상반신이라 사진을 950px 로 키운다
 *
 * case-01 과 모바일 순서가 달라 따로 둔다. PC 쪽은 CaseHeroSection 을 그대로 쓴다.
 */
export function Case02HeroSection({ hero }: { hero: BnaCaseDetail['hero'] }) {
  const titleLines = [hero.accent, ...hero.titleLines].filter(Boolean);
  const photo = hero.images[0];

  return (
    <section className="relative">
      {/* 모바일·태블릿 — 카피를 먼저 쌓고 사진을 맨 아래에 깐다 */}
      <Container className="pb-5 pt-2 text-center tb:pb-16 dt:hidden">
        <Typography
          as="p"
          variant="ko-body-03"
          tabletVariant="ko-body-01"
          weight="regular"
          className="text-center !tracking-normal text-gray-02"
        >
          {hero.eyebrow}
        </Typography>
        <Typography
        ...
        <div className="mt-7 flex items-center justify-center gap-4 tb:gap-6">
          <MobileWeightStep step={hero.before} tone="before" />
          <span
            aria-hidden="true"
            className="mt-4 h-0 w-0 border-y-[6px] border-l-[10px] border-y-transparent border-l-primary-main tb:mt-6"
        ...
        <p
          aria-hidden="true"
          className="mt-2 select-none whitespace-nowrap text-[56px] font-bold leading-none tracking-[-0.02em] text-[#D9DBD8] tb:text-[80px]"
        >
          {hero.watermark}
          {hero.watermarkUnit}
        </p>
        ...
        {/*
          높이를 정해두면 가로가 먼저 걸리는 사진에서 위쪽에 투명한 빈틈이 생긴다.
          그래서 폭을 꽉 채우고(Container 의 좌우 여백을 되돌린다), 태블릿에서
          사진만 커지지 않도록 폭에 상한만 둔다.
        */}
        {photo && (
      ...
      {/* PC — case-01 과 같은 배치. 사진만 키운다 */}
      <CaseHeroSection hero={hero} photoHeight={950} showMobile={false} />
    </section>
  );
}
...
/**
 * 사례 상세 한 벌. hero 는 사례마다 배치가 달라서 페이지가 직접 넘긴다.
 * topExtra 는 머리말과 히어로 사이에 끼울 것이 있을 때만 쓴다.
 */
